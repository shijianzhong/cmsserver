...
module.exports = {
  ...
  methods: {
    ...
    deletes(item) {
      ajax.call(this, "/deleteActiveSpecial", item, (data, err) => {
        this.loading = false;
        if (!err) {
          this.getCasesList();
          this.$message({
    ...
    saveArticle() {
      if (this.data.id != "") {
        ajax.call(this, "/updateActiveSpecial", this.data, (data, err) => {
          this.loading = false;
          if (!err) {
            this.getCasesList();
            this.$message({
        ...
        this.$refs.form.validate(v => {
          if (v) {
            this.visible = true;
            ajax.call(this, "/InsertActiveSpecial", this.data, (data, err) => {
              this.loading = false;
              if (!err) {
                this.getCasesList();
                this.$message({
